...
class PlayerActivity : ComponentActivity(), MPVLib.EventObserver {
      ...
      registerForActivityResult(ActivityResultContracts.GetContent()) { uri: Uri? ->
        if (uri != null) {
          try {
            ...
            if (fd != null) {
              val fdUri = "fd://$fd"
              if (playerViewModel.currentEngine.value == PlayerEngine.VLC) {
                MPVLib.command(arrayOf("sub-add", fdUri))
                Toast.makeText(this, "Subtitle Added to MPV! ✅", Toast.LENGTH_SHORT).show()
              } else {
                Toast.makeText(this, "Custom Subtitles require MPV Engine.", Toast.LENGTH_LONG)
                    .show()
              }
            }
          } catch (e: Exception) {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val savedThemeName =
        prefs.getString("app_theme", AppTheme.DEFAULT_DARK.name) ?: AppTheme.DEFAULT_DARK.name
    ...
    MPVLib.observeProperty("time-pos", 5) // 5 = MPV_FORMAT_DOUBLE
    MPVLib.observeProperty("duration", 5)
    MPVLib.observeProperty("pause", 3) // 3 = MPV_FORMAT_FLAG
    ...
    setContent {
      ...
      val currentPath =
          if (videoPaths.isNotEmpty() && currentIndex < videoPaths.size) videoPaths[currentIndex]
          else ""
  ...
  private fun playVideo(index: Int) {
    ...
    saveCurrentPlaybackPosition() // 🔥 আগের ভিডিওর পজিশন সেভ করলাম
    ...
    if (playerViewModel.currentEngine.value == PlayerEngine.EXO) {
      ...
      // 🔥 MPV Resume Fix: ফাইল লোড করার আগেই পজিশন বলে দিচ্ছি
      if (startPos > 3000L) {
        val startSec = startPos / 1000.0
        MPVLib.setOptionString("start", startSec.toString())
      } else {
        MPVLib.setOptionString("start", "none") // না হলে আগের ফাইলের পজিশন নিয়ে নেবে
      }
  ...
  override fun eventProperty(property: String, value: Boolean) {
    if (property == "pause" && playerViewModel.currentEngine.value == PlayerEngine.VLC) {
  ...
  override fun eventProperty(property: String, value: Double) {
    if (playerViewModel.currentEngine.value == PlayerEngine.VLC) {
      if (property == "time-pos") {
        playerViewModel.setCurrentPosition((value * 1000).toLong())
      } else if (property == "duration") {
        playerViewModel.setDuration((value * 1000).toLong())
      }
    }
  }
  ...
  override fun event(eventId: Int) {
    if (eventId == 7 && playerViewModel.currentEngine.value == PlayerEngine.VLC) {
  ...
  private fun handlePlaybackCompleted() {
    ...
    if (loopMode == LoopMode.ONE) {
      ...
      if (currentIndex < videoPaths.size - 1) playNext() else playVideo(0)
    } else {
  ...
  override fun onPause() {
    super.onPause()
    saveCurrentPlaybackPosition() // 🔥 Fix: ভিডিও বন্ধ হওয়ার আগেই সেভ!
  ...
  override fun onStop() {
    super.onStop()
    if (isFinishing) {
      saveCurrentPlaybackPosition() // 🔥 Fix: ভিডিও কাটার আগেই সেভ!
      try {
        MPVLib.setPropertyBoolean("pause", true)
      } catch (e: Exception) {}
      MPVLib.command(arrayOf("stop"))
